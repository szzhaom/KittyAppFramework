...
public class User extends IdTableObject<Long> implements UnuqieKeyCachable<Long>, kitty.kaf.session.SessionUser {
    ...
    public static final TableDef tableDef = new TableDef("t_user", "用户");
    ...
    static {
        ...
        tableDef.getColumns().put("userId", new TableColumnDef(3, "用户ID", "user_id", 4, 0, 0, false, "seq_user", false, 0, false, 1, null, null, null, null, true, false));
        tableDef.getColumns().put("userCode", new TableColumnDef(4, "用户编码", "user_code", 8, 30, 0, true, null, false, 0, false, 1, null, null, "正确格式为：以字母开头，长度在1~30之间，只能包含字符、数字和下划线", "^[a-zA-Z]\\w{1,30}$", false, false));
        tableDef.getColumns().put("userName", new TableColumnDef(5, "用户名", "user_name", 8, 255, 0, false, null, false, 0, true, 1, null, null, "由1~25550个字符组成,一个中文占2个字符", null, false, false));
        tableDef.getColumns().put("userPwd", new TableColumnDef(6, "密码", "user_pwd", 8, 32, 0, false, null, true, 0, false, 1, null, null, null, null, false, false));
        tableDef.getColumns().put("birthday", new TableColumnDef(7, "生日", "birthday", 7, 0, 0, false, null, false, 0, false, 1, null, null, null, null, false, false));
        tableDef.getColumns().put("gender", new TableColumnDef(8, "性别", "gender", 1, 3, 0, false, null, false, 0, false, 1, null, null, null, null, false, false));
        tableDef.getColumns().put("loginCount", new TableColumnDef(9, "登录次数", "login_count", 4, 0, 0, false, null, false, 0, false, 1, null, null, null, null, false, false));
    ...
    public List<Role> getRoleList() {
        ...
        if (roleList == null && roleIdList != null) {
            roleList = RoleHelper.roleMap.gets(roleIdList);
        }
        return roleList;
    }
    ...
    public List<Role> getOwnerRoleList() {
        ...
        if (ownerRoleList == null && ownerRoleIdList != null) {
            ownerRoleList = RoleHelper.roleMap.gets(ownerRoleIdList);
        }
    ...
    public void toJson(JSONObject json) throws JSONException {
        //[autogenerated:body(toJson) statements=7]
        super.toJson(json);
        ...
        json.put("text", userName);
    }

    @Override
    public void doReadFromStream(DataRead stream) throws IOException {
        //[autogenerated:body(doReadFromStream) statements=10]
    ...
    public void doWriteToStream(DataWrite stream) throws IOException {
        //[autogenerated:body(doWriteToStream) statements=10]
    ...
    public void readFromRequest(HttpRequest request, boolean isCreate) throws Exception {
        //[autogenerated:body(readFromRequest) statements=10]
        super.readFromRequest(request, isCreate);
        setId(tableDef.test("userId", request.getParameterLongDef("user_id", null), isCreate));
        setUserCode(tableDef.test("userCode", request.getParameter("user_code"), isCreate));
        setUserName(tableDef.test("userName", request.getParameter("user_name"), isCreate));
        setUserPwd(tableDef.test("userPwd", SecurityHelper.md5(request.getParameter("user_pwd")), isCreate));
        setBirthday(tableDef.test("birthday", request.getParameterDate("birthday"), isCreate));
        setGender(Gender.valueOf(request.getParameterInt("gender")));
        setLoginCount(tableDef.test("loginCount", request.getParameterLongDef("login_count", 0L), isCreate));
