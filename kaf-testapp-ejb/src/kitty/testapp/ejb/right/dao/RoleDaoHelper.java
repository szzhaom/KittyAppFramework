    ...
    public static void delete(Dao dao, Long loginUserId, List<Integer> idList) throws SQLException {
        ...
        if (idList != null && idList.size() > 0) {
            try {
                ...
                if (dao.query(1, SQLHelper.buildSelectOrListSql("t_user_role", "role_id", idList), idList).next()) throw new SQLException("删除角色失败：还有用户属于该角色，不能删除");
                dao.execute(SQLHelper.buildDeleteOrListSql("t_role_func", "role_id", idList));
                if (dao.query(1, SQLHelper.buildSelectOrListSql("t_user_owner_role", "role_id", idList), idList).next()) throw new SQLException("删除角色失败：还有用户拥有该角色，不能删除");
            } finally {
