...
public class DaoHelperClassGenerator extends ClassGenerator {
	...
	private void generateFindByUniqueKeyCode() {
		...
		ls.add(stmt);
		if (table.getForeignGenVarLists().size() > 0) {
			...
			for (ForeignKey fk : table.getForeignGenVarLists()) {
				if (fk.getIdListVarName() != null && !fk.getIdListVarName().trim().isEmpty()) {
					...
					if (i == 0) {
						...
						vars.add(ve);
						ls1.add(new ExpressionStmt(new VariableDeclarationExpr(new ReferenceType(
								new ClassOrInterfaceType("DaoResultSet")), vars)));
					} else
						ls1.add(new ExpressionStmt(new AssignExpr(new NameExpr("rset"), new MethodCallExpr(
								new NameExpr("dao"), "query", args), japa.parser.ast.expr.AssignExpr.Operator.assign)));
					List<Expression> args1 = new LinkedList<Expression>();
					args1.add(new IntegerLiteralExpr("0"));
					args = new LinkedList<Expression>();
					Table ft = generator.database.getTables().get(fk.getVarTable());
					args.add(new MethodCallExpr(new NameExpr("rset"), "get"
							+ ft.getPkColumn().getDataType().getShortName() + "List", args1));
	...
	private void generateEditCode() {
		...
		if (table.getForeignGenVarLists().size() > 0) {
			...
			List<Statement> fs = new LinkedList<Statement>();
			for (ForeignKey fk : table.getForeignGenVarLists()) {
				if (fk.getIdListVarName() != null && !fk.getVarTable().equals(fk.getTable().getName())) {
					// 多对多关系
					if (first) {
						ls.add(new ExpressionStmt(new MethodCallExpr(new NameExpr("dao"), "beginUpdateDatabase")));
						ls.add(new TryStmt(new BlockStmt(bs), null, new BlockStmt(fs)));
						fs.add(new ExpressionStmt(new MethodCallExpr(new NameExpr("dao"), "endUpdateDatabase")));
						first = false;
					}
					...
					if (t.getLocalCache() != null && !t.getLocalCache().trim().isEmpty()) {
						mce = new MethodCallExpr(new FieldAccessExpr(new NameExpr(t.getHelperClassName()), "local"
								...
								new NameExpr("dao"), "getLastModified", new ClassExpr(new ReferenceType(
										new ClassOrInterfaceType(t.getJavaClassName()))), new MethodCallExpr(
						...
						MethodCallExpr expr = new MethodCallExpr(new NameExpr(t.getHelperClassName() + "."
								...
								new MethodCallExpr(new NameExpr("o"), "get"
										+ StringHelper.toClassName(k.getVarBindColumn())));
						ls.add(new ExpressionStmt(expr));
					}
				}
			}
		}
	...
	private void generateFindByIdListCode() {
		...
		md.getParameters().add(
				new Parameter(new ReferenceType(new ClassOrInterfaceType("List", new ReferenceType(
						...
						new VariableDeclaratorId("idList")));
		md = JPHelper.addOrUpdateMethod(mainClass, md, true);
		List<Statement> ls = new LinkedList<Statement>();
		List<VariableDeclarator> vars = new LinkedList<VariableDeclarator>();
		MethodCallExpr mce = new MethodCallExpr(new NameExpr("dao"), "findByIdList", new LinkedList<Expression>());
		mce.getArgs().add(new ClassExpr(new ClassOrInterfaceType(table.getJavaClassName())));
		mce.getArgs().add(new NameExpr("idList"));
		vars.add(new VariableDeclarator(new VariableDeclaratorId("ret"), mce));
		ExpressionStmt stmt = new ExpressionStmt(new VariableDeclarationExpr(
		...
		if (table.getForeignGenVarLists().size() > 0) {
			...
			for (ForeignKey fk : table.getForeignGenVarLists()) {
				if (fk.getIdListVarName() != null && !fk.getIdListVarName().trim().isEmpty()) {
					...
					args.add(new MethodCallExpr(new NameExpr("rset"), "get"
							+ ft.getPkColumn().getDataType().getShortName() + "List", args1));
					ls1.add(new ExpressionStmt(new MethodCallExpr(new NameExpr("o"), ft.getPkColumn().getDataType()
							.getSetMethodName(varName), args)));
					i++;
				}
			}
		}
		autoGenerateStatements(md.getBody(), "autogenerated:body(findByIdList)", ls);
		ls = new LinkedList<Statement>();
		ls.add(new ReturnStmt(new NameExpr("ret")));
		autoGenerateStatements(md.getBody(), "autogenerated:return(findByIdList)", ls);
	}
	...
	private void generateFindByUniqueKeyListCode() {
		...
		md.getParameters().add(
				new Parameter(new ReferenceType(new ClassOrInterfaceType("List", new ReferenceType(
						new ClassOrInterfaceType("String")))), new VariableDeclaratorId("idList")));
		md = JPHelper.addOrUpdateMethod(mainClass, md, true);
		List<Statement> ls = new LinkedList<Statement>();
		List<VariableDeclarator> vars = new LinkedList<VariableDeclarator>();
		MethodCallExpr mce = new MethodCallExpr(new NameExpr("dao"), "findByNameList", new LinkedList<Expression>());
		mce.getArgs().add(new ClassExpr(new ClassOrInterfaceType(table.getJavaClassName())));
		mce.getArgs().add(new NameExpr("idList"));
		vars.add(new VariableDeclarator(new VariableDeclaratorId("ret"), mce));
		ExpressionStmt stmt = new ExpressionStmt(new VariableDeclarationExpr(
		...
		autoGenerateStatements(md.getBody(), "autogenerated:body(findByUniqueKeyList)", ls);
		ls = new LinkedList<Statement>();
		ls.add(new ReturnStmt(new NameExpr("ret")));
		autoGenerateStatements(md.getBody(), "autogenerated:return(findByUniqueKeyList)", ls);
	}
	...
	private void generateDeleteCode() {
			...
			for (ForeignKey fk : fks) {
				if (fk.getDelOption().equalsIgnoreCase("true")) {
					...
					Expression condition = new MethodCallExpr(new MethodCallExpr(new NameExpr("dao"), "query", args),
							"next");
					args = new LinkedList<Expression>();
					args.add(new StringLiteralExpr(fk.getPrompt()));
		...
		autoGenerateStatements(md.getBody(), "autogenerated:body(deleteCheck)", stmts, "autogenerated:body(delete)",
				true);
