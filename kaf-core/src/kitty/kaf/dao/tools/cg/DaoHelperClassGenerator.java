...
public class DaoHelperClassGenerator extends ClassGenerator {
	...
	private void generateQueryPageCode() {
		...
		AssignExpr ae = new AssignExpr(new NameExpr("ret"), new MethodCallExpr(new NameExpr("dao"), "queryPage", args),
	...
	private void generateFindByIdCode() {
		...
		if (table.getForeignGenVars().size() > 0) {
			...
			type.getTypeArgs().add(
					new ReferenceType(new ClassOrInterfaceType(table.getPkColumn().getDataType().getJavaClassName())));
			ClassOrInterfaceType type1 = new ClassOrInterfaceType("ArrayList");
			type1.setTypeArgs(new LinkedList<Type>());
			type1.getTypeArgs().add(
					new ReferenceType(new ClassOrInterfaceType(table.getPkColumn().getDataType().getJavaClassName())));
			vars = new LinkedList<VariableDeclarator>();
			vars.add(new VariableDeclarator(new VariableDeclaratorId("params"), new ObjectCreationExpr(null, type1,
					null)));
			ls1.add(new ExpressionStmt(new VariableDeclarationExpr(new ReferenceType(type), vars)));
			ls1.add(new ExpressionStmt(new MethodCallExpr(new NameExpr("params"), "add", new NameExpr("id"))));
			int i = 0;
